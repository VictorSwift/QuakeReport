...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            //lets traverse the json response from the root. the root is a JSON Object (bcux it is enclosed by curly braces)
            JSONObject baseJasonResponse = new JSONObject(SAMPLE_JSON_RESPONSE);
            //next node is the features key which is a Json Array (bcux it is enclosed by square braces).
            JSONArray earthquakeArray = baseJasonResponse.getJSONArray("features");
            //loop through each earthquake in the array
            for (int i = 0; i < earthquakeArray.length(); i++){
                //pull out the JSON object at the current position
                JSONObject currentEarthquake = earthquakeArray.getJSONObject(i);
                JSONObject properties = currentEarthquake.getJSONObject("properties");
                //access the individual values of the JSON Object by their keys
